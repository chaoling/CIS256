...
----Pseudocode-----
Hash the auction id combined with the inventory id,
combined with the winning bid amount to make a pseudo-unique value
store it in a dictionary with the key as the hash, and the node as the value

to delete, remove from id from dict
to add, add to dict with hashed value as key
to look up, dictionary lookup with a default of None

----Reflection----
the puyrpose of this code was to explore saving data
based on an arbitrary hashing method
I chose to use pythons builtin has function for readability
and speed
one issue I ran into was keeping track of bid ids
vs hashed ids while working in the code
this seems to run very quickly and store all of the nodes with few issues
'''
...
class HashTable(object):
    def __init__(self, size=DEFAULT_SIZE):
        ...
        self.tableSize = size
        self.data = [None for _ in range(self.tableSize)]
    ...
    def insert(self, bid: Bid)->None:
        ...
        if self.data[self.hash(bid.bidId)]:
            return
        #if no entry found for the key location
        # assign this node to the key position
        else:
            self.data[self.hash(bid.bidId)] = bid
    ...
    def printAll(self)->None:
        ...
        '''
        print(",".join([d.bidId for d in self.data if d]))
    ...
    def remove(self, bidId: str)->None:
        ...
        self.data[self.hash(bidId)] = None
    def search(self, bidId: str)->Bid:
        ...
        return self.data[self.hash(bidId)]
