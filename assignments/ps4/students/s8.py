...
class HashTable(object):
    ...
    def insert(self, bid: Bid) -> None:
        ...
        # assign this node to the key position
        if keynode is None:
            keymode = Node(bid, key=key)
            self.data[key] = keymode
        elif keymode is None:  # Check if keymode is None before accessing key
            keymode = Node(bid, key=key)
            self.data[key] = keymode
        else:
            #collision find the next empty position in the key location
            while keynode.next:
    ...
    try:
        with open(csvPath, 'r') as file:
            ...
            for row in reader:
                ...
                bid = Bid(id=entry.get('auctionid', ""),
                          title=entry.get('auctiontitle', None),
                          fund=entry.get('fund', ""),
                          amt=str2float(entry.get('winningbid', '0.0'),
                                        ['$', ',']))
                print(bid)
                bidTable.insert(bid)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p',
                        '--path',
                        default='eBid_Monthly_Sales_Dec.csv',
