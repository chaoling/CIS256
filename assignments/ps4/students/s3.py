...
class HashTable(object):
    ...
    def hash(self, id: int)->int:
        ...
        @param: id: an integer representing the bidId to be hashed
    ...
    def insert(self, bid: Bid)->None:
        ...
        #else in the next open node, add new node to the end of the chain
        else:
            while keynode.next:
                keynode = keynode.next
            keynode.next = Node(bid, key)
    ...
    def remove(self, bidId: str)->None:
        '''
        @param bidId the id of the bid to be removed from the hash table
        '''
        ...
        if keynode and keynode.key:
            if keynode.bid.bidId == bidId:   # if the node we're looking for is the first node in a linked (chained) list
                if keynode.next is None:    # if the node is the only one in the list
                    keynode.key = None      # erase the key reference, making the node unfindable
                    return
                else:
                ...
                while cur:
                    if cur.bid.bidId == bidId:   # if the node we're looking for is the first node in a linked (chained) list
                        pre.next = cur.next
                        cur = None
                        return
                    pre = cur
