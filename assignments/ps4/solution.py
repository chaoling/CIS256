...
class HashTable(object):
    ...
    def hash(self, id: int)->int:
        ...
        @param: key: an interger reperesting the bidId to be hashed
    ...
    def insert(self, bid: Bid)->None:
        ...
        if not keynode:
            #assign this node to the key location
            ...
            #collision! find the next empty slot in the key location
            while keynode.next:
    ...
    def remove(self, bidId: str)->None:
        ...
        if keynode and keynode.key:
            if keynode.bid.bidId == bidId:
                ...
                print(f"now removing node for {bidId}...")
                ...
                #collision occurred, search and remove the item in the chained linked list
                cur = keynode.next
                pre = keynode
                while cur:
                    if cur.bid.bidId == bidId:
                        #delete the current node by link the prev node with the cur->next
                        print(f"deleting the node: {cur.bid}")
                        pre.next = cur.next
                        cur = None
                        return
                    pre = cur
            ...
            print(f"no such node in the hashtable")
    ...
    def search(self, bidId: str)->Bid:
        ...
        if keynode and keynode.key and keynode.bid.bidId == bidId:
            return keynode.bid
        if not keynode or not keynode.key:
            return None #return empty bid
        tmp = keynode.next
        while tmp:
            if tmp.key and tmp.bid.bidId == bidId:
                return tmp.bid
            tmp = tmp.next
        return None    
