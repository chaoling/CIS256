...
def selectionSort(bids: List) -> None:
    ...
    '''
    n = len(bids)
    for i in range(n): #find min element
        min_idx = i #uses found min element assumes it is the smallest
        for j in range(i + 1, n): #finds smallest element
            if bids[j].title < bids[min_idx.title]: #if current element is smaller than previous element
               min_idx = j #updates making that the min element

        bids[i], bids[min_idx] = bids[min_idx], bids[i] #swaps min element
...
def quickSort(bids: List) -> None:
    ...
    def partition(bids: List, begin: int, end: int) -> int:
        ...
        low = begin #starts from beginning of list
        high = end #end of list
        while True:
    ...
    def qSort(bids: List, begin: int, end: int) -> None:
        ...
        if begin < end:
          pivot_index = partition(bids, begin, end) #partitions into 2 parts
          qSort(bids, begin, pivot_index) #sort left half
          qSort(bids,pivot_index + 1, end)#sort right half
