...
class Sudoku:
    ...
    def isAllowed(self, number, coord):
        ...
        #  This is numbered from 0-8, from left to right, top to bottom
        ...
        # check col
        col_values = []  # list to store column values
        for i in range(len(self.board)):
            col_values.append(self.board[i][col])
        if number in col_values:
            return False

        # check box
        box_values = []
        for i in range(3):  # put all the box values into a list to check
            for j in range(3):
                box_row = (3 * (box_num // 3)) + i  # finds the start M value based on box number then adds i
                box_col = (3 * (box_num % 3)) + j  # finds the start N value based on box number then adds j
                box_values.append(self.board[box_row][box_col])
        if number in box_values:
            return False
        else:
            return True
            ...
            for n in range(self.N):
                ...
                if self.board[m][n] == 0:
                    for try_num in range(1,10):
                        ...
                        if self.isAllowed(try_num, [m, n]):
                            self.board[m][n] = try_num
                            possible_solution = self.solve()
                    ...
                    if try_num == 9 and not possible_solution:
                        return False
