...
class Sudoku:
    ...
    def isAllowed(self, number, coord):
        ...
        #Checks if the number already exists in the 3x3 grids
        start_row = (row // 3) * 3
        start_col = (col // 3) * 3
...
#Solves the sudoku by using the recursive approach
    def solve(self):
        #FIXME: (2)
        '''
        ...
        for row in range(self.M):
          for col in range(self.N):
            ...
            if self.board[row][col] == 0:
              #Places a number 1-9 in the slot
              for num in range(1,10):
                ...
                if self.isAllowed(num, (row,col)):
                  ...
                  #If no solution is found it backtracks by resetting slot
