...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    plt.show()
    '''
    print("now plotting....",average)
    
    #plot the data using plotly
    fig = go.Figure()
    # Create and style traces
    fig.add_trace(go.Scatter(x=np.linspace(min(average)-10,max(average)+10,len(average)), y=average, name='Simple Virus',
                             line=dict(color='firebrick', width=4)))
    fig.update_layout(title='<b>Simple Virus Simulation</b>',
                   xaxis_title='Time Steps',
                   yaxis_title='Average Virus Population')
    fig.show()
    '''
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        count = 0
        for virus in self.viruses:
            #print(f"{type(drugResist)} : {virus.isResistantTo(drugResist[0])}")
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    """
    total_results, total_resistant_results = [],[]
    avg_t = []
    avg_r = []
    for i in range(numTrials):
        viruses = []
        ...
        for step in range(150):  #run first 150 steps without drug
            population[step] += patient.update()
            rpopulation[step] += patient.getResistPop(resistances)
        patient.addPrescription('guttagonol')
        for step in range(150, 300): #second stage from 150 to 300 steps
                population[step] += patient.update()
                rpopulation[step] += patient.getResistPop(resistances)

        total_results.append(population)
        total_resistant_results.append(rpopulation)

    #take the average of population and rpouplation
    for i in range(300):
        tsum = 0
        rsum = 0
        for j in range(numTrials):
            tsum += total_results[j][i]
            rsum += total_resistant_results[j][i]

        avg_t.append(tsum/numTrials)
        avg_r.append(rsum/numTrials)
...
if __name__ == "__main__":
    ...
    #simulationWithoutDrug(10, 100, .8, .2, 10)
    #simulationWithDrug(75, 100, 0.8, 0.1 ,{'guttagonol':False}, 0.8, 10)
    ...
    #simulationWithDrug(100, 1000,0.1, 0.05,{'guttagonol':False}, 0.005,10)
    #Even though they don't specify num trials in the instructions they do suggest up to 100 to make sure that the results are repeatable and representative.
    '''
    viruses = [
