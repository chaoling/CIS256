...
class BinarySearchTree(object):
    def __init__(self, root=None):
        ...
        def addNode(node: Node, bid: Bid) -> Node:
            if node is None:
                return Node(bid)
    ...
    def search(self, bidId: str) -> Optional[Bid]:
        def searchNode(node: Node, bidId: str) -> Optional[Bid]:
            if node is None:
                return None
    ...
    def remove(self, bidId: str) -> None:
        def removeNode(node: Node, bidId: str) -> Optional[Node]:
            if node is None:
                return None
    ...
    if self.root is None:
        ...
        # If tree is empty, assign bid as root
        if self.root is None:
            self.root = Node(bid)
        else:
    ...
    def remove(self, bidId: str)->None:
       def findMin(node: Node) -> Node:
        ...
        if int(bidId) < int(node.bid.id):
            node.left = removeNode(node.left, bidId)
        elif int(bidId) > int(node.bid.id):
            node.right = removeNode(node.right, bidId)
        else:
            ...
            node.right = removeNode(node.right, minNode.bid.id)
